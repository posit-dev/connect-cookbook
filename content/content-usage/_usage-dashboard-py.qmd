...
# 3. Specify if the summary should show only the Top N users for the content, or all users (as indicated by `np.nan`). Default is `np.nan` so that all users are shown.
top_n = 20
top_n = np.nan
...
client = Client() #TODO: connect.Client() not working but desired
```

```{python}
#| label: Get content info and usage details
...
# visits = pl.DataFrame(client.visits.find(content_guid=content_guid))
