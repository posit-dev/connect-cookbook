...
Apache Airflow is self-described as “a platform created by the community to programmatically author, schedule, and monitor workflows.” In other words, we can use Apache Airflow to orchestrate custom workflows. This includes but is not limited to, a machine learning development workflow.

A machine learning development workflow is designed, in part, to create a machine learning model systematically. A robust workflow often has many additional components, but for this tutorial, let’s concentrate on producing a model that we can use to perform a classification.

After training the model, we’ll deploy an application to Connect that uses the model to predict a classification based on user input.

Then, we’ll refactor our machine learning development workflow for Apache Airflow and programmatically update our application to use different models.

## Machine Learning Development Workflow

### Data Exploration

The first step in our machine learning development workflow is data exploration. For this tutorial, let's use the [`palmerpenguins`](https://allisonhorst.github.io/palmerpenguins/) dataset.
...
Let's try to build a machine learning model which correctly determines the species using other information available in the dataset. The `species` column contains three unique values: *Adelie*, *Chinstrap*, and *Gentoo*. For simplicity, let's use the four numerical columns as features in our dataset: `bill_length_mm`, `bill_depth_mm`, `flipper_length_mm`, and `body_mass_g`.

::: {.callout-tip}
To learning more bout this datset, check out ["Introduction to palmerpenguins"](https://allisonhorst.github.io/palmerpenguins/articles/intro.html)
:::
...
Next, split `X` and `y` into training and testing sets. Let's use the testing set to evaulate the performance of our model.
...
Finally, let's write the encoder and pipeline to Connect. For additional information on configuring a `board_connect` with your credentials, check out the [`pins` documentation](https://rstudio.github.io/pins-python/).
