...
Running the following code will generate a table of all content that is set without restriction. The information includes URL of the content on Connect and the user name. 
...
response <- GET(paste(CONNECT_SERVER,"__api__/v1/content",sep = ""),
              add_headers(Authorization = paste("Key", CONNECT_API_KEY))
              ,query = list(include = "owner"))


content <- (httr::content(response, as = "text")) |>
   jsonlite::fromJSON(simplifyDataFrame = T) |>
   unnest(cols = c(owner),names_sep = '$')
...
if not CONNECT_API_KEY:
        raise ValueError("ERROR: valid CONNECT API key is required")

# check if Server URL is valid
if not CONNECT_SERVER:
        raise ValueError("ERROR: valid CONNECT Server URL is required")
...
response = requests.get(
            CONNECT_SERVER + "__api__/v1/content/", headers=headers
            , params=params
            )
...
The list above also identifies the users who published the content who can be reached out to if there is a need to modify the access.