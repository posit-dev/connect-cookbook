...
purrr::walk(content$guid, ~ {
  en <- get_environment(content_item(client, .x))
  if (env_var %in% en$env_vars) {
    set_environment_new(en, "{env_var}" := new_value)
    print(paste("Updated", env_var, "in Content", .x))
  } else {
    print(paste("Environment variable", env_var, "not found in Content", .x))
  }
})
