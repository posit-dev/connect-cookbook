---
title: 13.3 Promoting Content from Staging to Production
---

## Problem

You’re using with two Connect servers for staging and production, and want to promote deployed content from one to the other
...
Next, obtain the bundle you want to promote using a bundle identifier (id), then download the bundle archive. See ["Finding Bundles"](../../07-bundles/02-finding-bundles) and ["Downloading Bundles"](../../07-bundles/06-downloading-bundles) for additional information.
...
Finally, wait for the deployment task to complete. The `wait_for()` method will watch the deployment process and return once it completes. Check the status and returning an error if it fails. See ["Viewing Task Information"](../../08-tasks/01-viewing-task-information/) for additional information.
