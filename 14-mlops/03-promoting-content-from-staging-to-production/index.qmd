...
Finally, call the `wait_for` method on the deployment task. The `wait_for` method watches the deployment process and returns once it completes. Check the status and returning an error if it fails. See [Viewing Task Information](../../08-tasks/01-viewing-task-information/) for additional information.
